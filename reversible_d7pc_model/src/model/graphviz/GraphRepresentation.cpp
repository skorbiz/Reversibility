...
GraphRepresentation::GraphRepresentation(std::shared_ptr<model::Instruction> root) :
		_maxRank(-1)
{
...
std::vector<std::shared_ptr<model::Instruction> > GraphRepresentation::getInstructions()
{
	std::vector<std::shared_ptr<model::Instruction> > ret;
	for ( std::map<std::shared_ptr<model::Instruction>, Node>::iterator it = _nodes.begin(); it != _nodes.end(); ++it)
		ret.push_back(it->second.instruction);
	return ret;
}


std::vector<std::shared_ptr<model::Instruction> > GraphRepresentation::getInstructionOfRank(int rank)
{
	std::vector<std::shared_ptr<model::Instruction> > ret;
	for ( std::map<std::shared_ptr<model::Instruction>, Node>::iterator it = _nodes.begin(); it != _nodes.end(); ++it)
...
std::vector<std::shared_ptr<model::Instruction> > GraphRepresentation::getOmniDirectionalEdges()
{
	std::vector<std::shared_ptr<model::Instruction> > ret;
	for ( std::map<std::shared_ptr<model::Instruction >, Node>::iterator it = _nodes.begin(); it != _nodes.end(); ++it)
	{
		std::shared_ptr<model::Instruction> current = it->second.instruction;
		std::shared_ptr<model::Instruction> next = current->getInstructionNext();

		if(next == nullptr)
			continue;

		std::shared_ptr<model::Instruction> isCurrent = next->getInstructionPrevious();
...
std::vector<std::shared_ptr<model::Instruction> > GraphRepresentation::getForwardExlusivEdges()
{
	std::vector<std::shared_ptr<model::Instruction> > ret;
	for ( std::map<std::shared_ptr<model::Instruction>, Node>::iterator it = _nodes.begin(); it != _nodes.end(); ++it)
	{
...
std::vector<std::shared_ptr<model::Instruction> > GraphRepresentation::getBackwardExclusivEdges()
{
	std::vector<std::shared_ptr<model::Instruction> > ret;
	for ( std::map<std::shared_ptr<model::Instruction>, Node>::iterator it = _nodes.begin(); it != _nodes.end(); ++it)
	{
		std::shared_ptr<model::Instruction> current = it->second.instruction;
		std::shared_ptr<model::Instruction> prev = current->getInstructionPrevious();

		if(prev == nullptr)
			continue;

		std::shared_ptr<model::Instruction> isCurrent = prev->getInstructionNext();
...
void GraphRepresentation::buildGraph(std::shared_ptr<model::Instruction> ins)
{
...
void GraphRepresentation::buildNodes(std::shared_ptr<model::Instruction> ins)
{
	if(ins == nullptr)
		return;
...
void GraphRepresentation::buildEdges(std::shared_ptr<model::Instruction> ins)
{
	if(ins == nullptr)
		return;
	...
	std::shared_ptr<model::Instruction> next = ins->getInstructionNext();
	std::shared_ptr<model::Instruction> prev = ins->getInstructionPrevious();
...
void GraphRepresentation::rankGraph(std::shared_ptr<model::Instruction> ins, int rank)
{
	if(ins == nullptr)
		return;
