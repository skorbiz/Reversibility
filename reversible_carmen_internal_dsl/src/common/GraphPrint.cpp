...
GraphPrint::GraphPrint(dsl::BaseProgram aAssemblyProgram) :
		_graph(aAssemblyProgram)
{
	openFile();
	writeHeader();
	print();
...
void GraphPrint::print()
{
	...
	{
		std::vector<std::shared_ptr<dsl::Instruction> > ins = _graph.getInstructionOfRank(r);
			_outputFile << "\n {rank = same; \"r" << r << "\"";
			for(unsigned int i = 0; i < ins.size(); i++)
				_outputFile << " \"" << ins[i]->getID() << "\"";
			_outputFile << "};";
	}
	...
	std::vector<std::shared_ptr<dsl::Instruction> > instructions = _graph.getInstructions();
	for(unsigned int i = 0; i < instructions.size(); i++)
		writeNode(instructions[i]);

	std::vector<std::shared_ptr<dsl::Instruction> > edgeDouble = _graph.getOmniDirectionalEdges();
	for(unsigned int e = 0; e < edgeDouble.size(); e=e+2)
		writeConnectionDouble(edgeDouble[e],edgeDouble[e+1]);

	std::vector<std::shared_ptr<dsl::Instruction> > edgeForward = _graph.getForwardExlusivEdges();
	for(unsigned int e = 0; e < edgeForward.size(); e+=2)
		writeConnectionForward(edgeForward[e],edgeForward[e+1]);

	std::vector<std::shared_ptr<dsl::Instruction> > edgeBackward = _graph.getBackwardExclusivEdges();
...
{
	std::string filePath = common::getPath2trunk() + "/dsl/outputs/";
...
{
	std::string path = common::getPath2trunk();
	_outputFile << "\n }" << std::endl;
	std::string input = path + "/dsl/outputs/graph.dot";
	std::string output = path + "/dsl/outputs/graph.png";
...
std::string GraphPrint::generateNodeProperties(std::shared_ptr<dsl::Instruction> a)
{
...
std::string GraphPrint::generateNodeArgument(std::shared_ptr<dsl::Instruction> a)
{
	std::string f;
	std::string b;
	b += "<BR/>";
...
void GraphPrint::writeNode(std::shared_ptr<dsl::Instruction> a)
{
	std::string nodeID = a->getID();
...
void GraphPrint::writeConnectionDouble(std::shared_ptr<dsl::Instruction> a, std::shared_ptr<dsl::Instruction> b)
{
...
void GraphPrint::writeConnectionForward(std::shared_ptr<dsl::Instruction> a, std::shared_ptr<dsl::Instruction> b)
{
	writeConnectionArgumented(a,b);
}

void GraphPrint::writeConnectionBackwards(std::shared_ptr<dsl::Instruction> a, std::shared_ptr<dsl::Instruction> b)
{
	writeConnectionArgumented(a,b, "[ arrowhead=onormal ]");
}

void GraphPrint::writeConnectionArgumented(std::shared_ptr<dsl::Instruction> a, std::shared_ptr<dsl::Instruction> b, std::string argument)
{
	writeConnectionArgumented(a->getID(),b->getID(),argument);
}
