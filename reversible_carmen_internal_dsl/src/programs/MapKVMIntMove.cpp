...
void MapKVMIntMove::buildExpandedModel()
{
	...
	rw::math::Q pos_screwdriver_take_infront(6, 0.849006658158743, -0.9276855492371849, 1.4052383056030697, -0.4775813335093756, -3.8633762620935084, -1.5707727416277333);
	...
	rw::math::Q pos_tube_take_at 	(6, 2.61505198, -1.2303603, 1.69783592, -2.0270512, -1.5772302, 2.60829782 );
	rw::math::Q pos_tube_take_in 	(6, 2.72028470, -1.2718833, 1.75522851, -2.0429990, -1.5760090, 2.71330547 );
	rw::math::Q pos_tube_take_above 	(6, 2.71761059, -1.3337281, 1.64773416, -1.8737214, -1.5764640, 2.71158146 );
	...
	rw::math::Q pos_tube_rotate_at	(6, 2.07467913, -1.7643936, 2.15112400, -1.9333995, -1.5581501, 2.09054541 );
	rw::math::Q pos_tube_rotate_in	(6, 2.07476305, -1.7322056, 2.20173406, -2.0161636, -1.5578988, 2.09018635 );
	rw::math::Q pos_tube_rotate_out	(6, 2.02108335, -1.6431134, 2.12677145, -2.0293291, -1.5592396, 2.03641676 );
...
//		uncall("machine_open");
	...
	rw::math::Q nutGrasp_at 			(6, 2.499607, -1.66887, 2.016528, -1.87577, -1.53667, 2.493259);
	rw::math::Q nutGrasp_angle_up 		(6, 2.386445, -1.55364, 1.832422, -1.05189, -0.93596, 2.103391);
	rw::math::Q nutGrasp_angle_down 	(6, 2.625256, -1.62003, 2.158690, -2.53937, -2.22100, 2.474643);
...
////		intMove(outside_farUP).
////		intMove(outside_far).
////		intMove(outside_close).
////		forceMode(forward,deactivate).
////		wait(2).
////		forceMode(deactivate,deactivate).
////		check(inside_nut);
////		intMove(inside_deep).
////		intMove(above_inside);
////
////	sequence("pick_nuts").
////		call("pick_nut").
////		call("fixat_nut");
...
//
	...
	std::string act_nut_basepath 		= common::getPath2trunk();
	...
	sequence("action_pih1").
		move(outside_far).
		intAction(act_nut_path, act_nut_tcp, act_nut_fix1, pos_common).
		move(above_inside);

	sequence("action_pih2").
		move(outside_far).
		intAction(act_nut_path, act_nut_tcp, act_nut_fix2, pos_common).
		move(above_inside);
